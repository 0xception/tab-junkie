...
| C-15 | Browser-API rejection-string contract verification | If error classification depends on the rejection's `message` substring (e.g., a `_classifyError` helper or any `err.message.includes('...')` predicate), R2 MUST verify the actual Chrome rejection-string format via a 30-second SW REPL probe (e.g., `chrome.tabs.move(99999, ...).catch(e => e.message)` to capture the real "No tab with id: N" string) and document the verified string in the chapter; mock layers in `tests/chrome-mock.js` MUST emit the verified format going forward. The Sprint 42 B-041 R4 fix-round is the blocking precedent — `_classifyError` was tuned to chrome-mock synthetic strings (`Tab N not found`); real Chromium rejections use a different format (`No tab with id: N`). All three R4 reviewers (security M-1 + code M-4 + qa root-cause) converged on the gap; the tab-gone integration test was illusory until M-1/M-4 fix-round re-aligned the predicate set against verified strings. |

**Fix-scope test-assertion enumeration (mandatory subsection in every R2 chapter declaring a contract change)**

When R2 declares a contract modification (DOM structure, ARIA contract, message shape, CSS class semantics, selector contract, **CSS-token invariants asserted in test files (regex-pin tests on `shared/themes.css`, structural assertions on `--<token>` values, count-of-N assertions on token declarations)**, **DOM-structural pins on shared surfaces (fieldset counts, section orders, selector-coverage enumerations, legend lists on the settings/sidepanel/newtab/popup pages)**, etc.), the chapter's fix-scope table MUST include a "pre-existing test assertions to update" enumeration listing every test `file:line` that asserts the pre-change contract. R3 cannot start until this enumeration is present and verified by [solution-architect].

Format:
`tests/foo.test.js:NN — asserts <pre-change contract>; update to <post-change contract>`

Three blocking precedents:
...
- **Sprint 42 B-041 D-1** — adding a new "Chrome Integration" `<fieldset>` to `settings/settings.html` broke `tests/b091-settings-page.test.js:313` (5-fieldset count pin) and `:325` (Display/Layout/Groups/Theme/Data section-order pin); R2 §67.7 fix-scope enumeration missed both. R3 self-corrected at build time but the catch was three-occurrences late. The DOM-structural-pin addition above is the direct response to this third-occurrence precedent.
...
- **Shared-surface consumer inventory (mandatory subsection in R2 chapters that introduce a new consumer of shared DOM/state)**: when an R2 chapter introduces a new consumer of any shared `#settings-*` / `#sidepanel-*` / `#newtab-*` / `#popup-*` element OR shared module-level state (timers, mirrors, in-memory maps, generation counters), the chapter MUST include a "shared-surface consumer inventory" subsection enumerating: (a) every existing consumer of the surface; (b) the planned new consumer; (c) the coordination mechanism (single-owner / refcount / singleton helper / etc.). The Sprint 42 B-041 R4 H-2 ghost-timer race is the blocking precedent — `settings-chrome-sync.js` was added as a second consumer of `#settings-toast` without inventorying the existing `settings-import-export.js:40` `_toastTimer` consumer; both modules independently `setTimeout`'d against the shared DOM, racing each other. R4 [code-reviewer] caught the issue at HIGH; the fix-round extracted `settings/settings-toast-timer.js` as a singleton-timer helper. Inventory at R2 prevents this class.
