...
### Internal
- **End-to-end cold-start reconciliation integration test (B-174)** — regression safety net verifying the full cold-start claim-reconciliation flow end-to-end; establishes a baseline that all subsequent B-173 sub-items run against.
- **Shared tab↔item resolver extracted (B-175)** — 10 duplicated, subtly-divergent tab↔item matching sites replaced with a single canonical resolver in `shared/tab-item-resolver.js`. Eliminates the class of bugs where one matching site was updated but siblings were not.
- **`background/floating-groups.js` split into 5 modules (B-176)** — the 1,344-line monolith decomposed into `floating-store.js`, `floating-assoc.js`, `floating-prune.js`, `floating-render.js`, and `floating-groups.js` (re-export barrel). Internal cohesion only; no API or behavior change.
- **`onReplaced`/`onRemoved` event fan-out named as documented primitives (B-177)** — anonymous event fan-out handlers are now named functions with JSDoc contracts, making the event-to-handler mapping grep-visible.
- **`reconcileClaims` decomposed into 4 named phase functions (B-178)** — the 181-line monolith split into `phase1PreMark`, `phase2ClaimByUrl`, `phase3DriftFallback`, `phase4DropDrift`; each phase has an independent JSDoc contract. No behavior change.
- **Store cutover: `tj:itemClaims` is now sole persisted claim store (B-179)** — `tj:tabClaims` (session store) retired; `floatingGroups.liveTabId` demoted to a derived cache populated at runtime. A one-cold-start compatibility shim migrates any in-flight session claims to durable on first run after update (write failure never strands claims). No additional schema bump — operates within v8 partition layout.
- **Schema v8 → v9 eager `tj:floatingGroups` normalization (B-180)** — paired `KNOWN_VERSION` 8→9 + `defaultShape(PARTITION_META)` 8→9 + first non-no-op `MIGRATION_STEPS` entry: a one-time eager rewrite normalizing every `tj:floatingGroups` record to the canonical v4 STABLE-field shape (`parentItemId` + `floatingTabId` + `sortOrder`). Additive + idempotent + drops no record; `liveTabId` is NOT fabricated — runtime (`reassociateFloatingGroups` / the B-175 resolver) recomputes it. Meta version bump + floatingGroups rewrite commit atomically in a single multi-partition `writeTransaction`. Position/URL recovery tiers + tolerant read-validator deliberately retained (orphan-risk mitigation; removal deferred to B-183).
- **Test count**: 2099 (S46 baseline) → 2127 PASS in S47 close (+28). Zero regressions.
