...
- **Priority**: P2
- **Status**: **R5 COMPLETE — UAT READY (2026-06-03)** — Test gap analysis identified 5 uncovered areas vs §73.15 + R5 charter: public-API end-to-end cold-start contract (`isClaimsReady` + `getItemIdForTab` post-reconcile per §73.15 T7 verbatim "no Phase 2/3 URL-inference observable"); Q3 MSG_DEMOTE_ITEM ordering invariant (T15 verified end-state only, not the deleteItem → clearDrift → saveFloatingGroups → releaseClaimByTab sequence); zero-tab cold start (empty liveTabIndex + non-empty durable → sessionMatches false → W-1 self-heal to empty entries); 50-entry scale (correctness under non-trivial entries map; well within §73.14 perf envelope); claim-then-immediate-release race (W-4 followed by W-2 under serial writeTransaction). Five gap-filler cases added (T17 cold-start surface, T18 demote-ordering, T19 zero-tab, T20 scale, T21 race). b167 suite expanded 17 → 22 cases, all PASS. **Full suite 2087 → 2098 PASS, zero regressions.** UAT script written into `docs/findings/sprint-46.md` "B-167 UAT plan — Durable claim identity (R5)" — 4 scenarios (UAT-1 extension reload preserves binding / UAT-2 browser-restart backstop / UAT-3 corrupt-partition recovery / UAT-4 rollback safety), every step UI-observable per S45 retro discipline (no SW-console state queries, no chrome.storage.local dumps — UAT-3 has one DevTools input line to seed malformed storage that the UI cannot create, but verification remains UI-only). Ready for user-driven UAT execution against the loaded unpacked extension.\n- **Status (R4 fix-round, retained)**: R4 FIX-ROUND COMPLETE (2026-06-03) — CONV-1 convergent finding (code M-2 + security M-1 + qa M-1) landed: all 4 durable helpers (`durableMirrorFullReplace`, `durableUpsertEntry`, `durableDeleteEntry`, `durableRemapEntry`) now resolve `tag = _sessionTag || (cur && cur.sessionTag) || ''` ONCE at the top of the mutator and stamp the SAME value onto BOTH the partition-level `sessionTag` field AND the per-entry `sessionTag` stamp — closes the divergence window during `ensureSessionTag` write-failure paths. `durableUpsertEntry` adds the qa M-1 fail-fast guard (`if (!_sessionTag) return;` after the best-effort `ensureSessionTag()` retry) so a triple-failure path cannot produce a silent empty-tag durable record. qa M-2: T6 read-back assertion added — verifies `tj:itemClaims` self-heals from corrupt input via W-1 full-replace. L-2: `@typedef TxOp` partition union in `background/storage/partitions.js:32` widened to `'recency'|'itemClaims'`. L-3: stale `KNOWN_VERSION = 7 (B-148 §3.1, S44)` comment in `background/storage/shapes.js:114-116` updated to `KNOWN_VERSION = 8 (B-167 §73.3.1, S46)`. New T16 regression-guard test pins the CONV-1 unified-tag invariant via the new `__setSessionTagForTest` hatch. Deferred to P3 backlog: code M-1 (cosmetic double-readPartition), qa M-3 (mock failure-injection infra), remaining LOWs. **Tests 2086 → 2087 PASS, zero regressions.** Contract-vs-implementation diff gate self-check: clean on every fixed predicate. Ready for R5 [test-engineer].\n- **Status (R3, retained)**: R3 BUILD COMPLETE (2026-06-03) — durable identity layered above (`prePopulateClaimsFromDurable`) and below (W-1..W-5 mirror) the existing Phase 1/2/3/4 inference pipeline per §73. Schema v7→v8 paired bump (`KNOWN_VERSION` 7→8 + `defaultShape(PARTITION_META)` literal 7→8 + new `PARTITION_ITEM_CLAIMS` constant in `ALL_PARTITIONS`); v7→v8 no-op `MIGRATION_STEPS` entry; allow-list validator `isItemClaims` (C-7 forward-compat for B-172). `sessionMatches` predicate threshold 0.5 (Q2 PICK). `releaseClaimByTab` updated for W-2 best-effort sequential durable-delete (Q3 PICK). 5 enumerated pre-existing test pins updated PLUS 1 R2-VERIFY-caught extra pin in `tests/migration-steps.test.js:88` (R3 grep audit per §73.9.2 caught it; not a STOP-and-escalate — fix-scope completion). 16 new b167-* tests cover T1-T15 (T1 defaultShape, T2/T2b validator, T3 forward-compat, T4 paired-bump, T5 sessionMatches threshold, T6 corrupt-data graceful degradation, T7 S-1 happy path, T8 S-2 backstop + fresh tag, T9 W-2 dual-clear, T10 W-3 upsert, T11 W-4 upsert, T12 W-5 tabId remap preserves claimedAt+sessionTag, T13 B-149 regression, T14 B-163 regression, T15 MSG_DEMOTE_ITEM clears durable). Tests 2069 → 2085 PASS, zero regressions.
...
- **Files Changed**:
  ...
  - `tests/b167-durable-claim-identity.test.js` (NEW + R5 expansion) — T1–T16 (17 cases) covering all R1 ACs + S-1/S-2 scenarios + W-1..W-5 mirror + graceful degradation + B-149/B-163 regression guards + CONV-1 unified-tag invariant; **R5 added T17–T21 (5 cases)**: T17 public-API cold-start surface, T18 MSG_DEMOTE_ITEM ordering, T19 zero-tab cold start, T20 50-entry scale, T21 claim-then-release race. Total b167 suite: 22 cases.
  - `docs/findings/sprint-46.md` — B-167 UAT plan appended (4 UI-observable scenarios, S45 retro discipline).
...
- **Priority**: P2
- **Status**: **R5 COMPLETE — UAT READY (2026-06-03)** — Test gap analysis identified 4 uncovered areas: M-2 rapid-click guard, M-1 explicit `durationMs:3000` toast contract, L-1 `focus({ preventScroll: true })` keyboard focus, prefers-reduced-motion CSS path. Six gap-filler cases added (T8/T9/T9b/T10/T10b/T11). T9b + T10b are source-truth regex assertions on `sidepanel/sidepanel.js` that pin the live code against inline-shim drift. b168 suite expanded 12 → 18 cases, all PASS. **Full suite 2087 → 2093 PASS, zero regressions.** UAT script written into `docs/findings/sprint-46.md` "B-168 UAT plan — Jump to active window (R5)" — 7 scenarios (UAT-1 toolbar / UAT-2 Alt+W keyboard / UAT-3 no-match toast / UAT-4 rapid-click / UAT-5 WINDOW_ID_NONE Alt-tab guard / UAT-6 keyboard focus after jump / UAT-7 prefers-reduced-motion), every step UI-observable per S45 retro discipline. Ready for user-driven UAT execution against the loaded unpacked extension.
- **Assigned To**: [product-manager] (R1) → [solution-architect] Sonnet (R2) → [frontend-engineer] (R3 + R4 fix-round) → [test-engineer] (R5) → user (UAT execution)
- **Blockers**: none
- **Files Changed (R5)**: `tests/b168-jump-to-active-window.test.js` (+6 cases T8/T9/T9b/T10/T10b/T11), `docs/findings/sprint-46.md` (B-168 UAT plan section appended).
