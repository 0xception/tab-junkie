 ...
 */
test('B-137 R5 L-3: defaultShape(PARTITION_META).schemaVersion === 7 (B-148 §3.1 paired-bump invariant)', () => {
  const shape = defaultShape(PARTITION_META);
  assert.equal(shape.schemaVersion, 7,
    'defaultShape(PARTITION_META).schemaVersion must be the literal 7 — bump this when KNOWN_VERSION bumps');
