 ...
 */
test('B-137 R5 L-3: defaultShape(PARTITION_META).schemaVersion === 9 (B-180 §74.10 paired-bump invariant)', () => {
  const shape = defaultShape(PARTITION_META);
  assert.equal(shape.schemaVersion, 9,
    'defaultShape(PARTITION_META).schemaVersion must be the literal 9 — bump this when KNOWN_VERSION bumps');
