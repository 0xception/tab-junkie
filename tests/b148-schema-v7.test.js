...
/* B-148 v6→v7 floor + B-180 v8→v9 ceiling: this file now pins the
   current KNOWN_VERSION literal as a paired-bump regression guard. The
   v7 floor was B-148's contribution; the v9 ceiling is B-180's (the first
   eager data migration — B-167's v8 was the prior ceiling). */
test('B-148 §3.1 (B-180 §74.10 bump): KNOWN_VERSION === 9', () => {
  assert.equal(KNOWN_VERSION, 9);
});

test('B-148 §3.1 (B-180 §74.10 bump): defaultShape(PARTITION_META).schemaVersion === 9', () => {
